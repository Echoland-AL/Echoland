...
async function saveFriendsData(personId: string, data: { friends: Array<{ id: string; strength?: number; addedAt?: string }> }): Promise<void> {
  await fs.mkdir(FRIENDS_DIR, { recursive: true });
...
async function ensureHomeArea(account: Record<string, any>) {
  ...
  try {
    ...
    if (!exists) {
      ...
      console.log(`[AREASEARCH] Added home area ${areaId} (${areaName}) to ${account.screenName}'s created areas list`);
    }
  } catch (error) {
...
<body>
  ...
  .post("/area", async ({ body, cookie }) => {
    ...
    try {
      ...
      console.warn("⚠️ Could not update active profile with new owned area:", e);
    }
